...
Self-RAG's decision whether to retrieve depends on the nature of the question and the model's internal knowledge boundaries. Questions within the model's training distribution can be answered directly, while questions about specific, time-sensitive, or private information require external retrieval. The table below illustrates the decision logic across common question types.
...
Extracting relationships between entities is the step that transforms raw text into a navigable knowledge graph. The choice of extraction method involves a trade-off between precision, cost, and the availability of labeled training data. Simpler rule-based approaches work for predictable patterns, while LLM-based extraction handles diverse and complex relationships at higher cost.
...
Microsoft GraphRAG introduced four architectural components that together enable both entity-level precision and corpus-level summarization. Hierarchical clustering via the Leiden algorithm is the critical differentiator, allowing the system to answer questions at multiple levels of abstraction.
...
GraphRAG provides two fundamentally different retrieval modes, each optimized for a different class of questions. Local search starts from specific entities and traverses their graph neighborhoods, while global search operates over precomputed community summaries using a map-reduce pattern. Choosing the right mode depends on whether the question targets specific entities or asks about corpus-wide themes.
...
RAPTOR's multi-level index enables the retrieval system to match the granularity of the answer to the granularity of the question. Detailed technical questions retrieve specific chunks, while high-level conceptual questions retrieve summaries that would never exist in a flat index. The table below illustrates how different question types map to RAPTOR's abstraction levels.
...
**LLM-as-judge:** A strong model (GPT-5.4, Claude) is shown the question, context, answer, and evaluation criteria. It returns a score (0-1 or 1-5) and justification. This enables rapid evaluation of thousands of examples without manual effort.
...
Choosing a vector database is an infrastructure decision with long-term implications for cost, operational complexity, and scaling ceiling. Each option occupies a distinct niche: fully managed services minimize ops burden, while self-hosted solutions provide control and cost predictability at scale. The table below maps popular databases to their strongest use cases.
...
The GraphRAG landscape now offers a spectrum of complexity-cost trade-offs. Standard GraphRAG delivers the best query latency through precomputed summaries but requires expensive upfront indexing. LazyGraphRAG and LinearRAG reduce that upfront cost at the expense of per-query compute. The right choice depends on corpus volatility, query volume, and budget constraints.
...
**Contextual Retrieval** is another name for the Context-Aware RAG technique described above. Combined with hybrid search (BM25), it reduces retrieval errors by 67%.
...
Each recent technique targets a specific failure mode in the retrieval pipeline: context loss during chunking, OCR errors in visual documents, or retrieval misses on non-obvious connections. They are not mutually exclusive and can be layered for cumulative benefit. The table below summarizes what each technique solves and when the cost trade-off is justified.
