...
**SWE-bench Verified** — evaluates autonomous coding agent capability on real GitHub issues from popular open-source projects. Each task is a real bug report or feature request with a test suite. The model must understand the codebase, localize the issue, write a fix, and pass all tests. The acceleration of scores has been dramatic: 72% (mid-2025) → 79.2% (early 2026) → 87.6% (Claude Opus 4.7, April 2026) → 88.7% (GPT-5.5 Instant, May 2026) → 88.6% (Claude Opus 4.8, May 28 2026) → **95.0%** (Claude Fable 5, June 9 2026, independently confirmed by vals.ai; **model suspended June 13 due to export controls** — see [[../14_Security_Safety/03_Agent_Security|Agent Security]]). DeepSeek V4 scores 80.6% (open weights, MIT). Mistral Medium 3.5 scores 77.6%. Fable 5 at 95% means the benchmark is approaching its practical ceiling — with solution leakage in ~33% of tasks and pretraining contamination in ~94%, the remaining 5% gap likely reflects benchmark noise more than model limitation. OpenAI officially stopped reporting Verified scores (May 2026) due to confirmed contamination — see Benchmark Reliability section.

**SWE-bench Pro** — a harder variant introduced to address Verified's reliability issues, now the recommended replacement after OpenAI abandoned Verified (May 2026). Claude Fable 5 leads at **80.3%** (vendor-reported, +11pp over Opus 4.8's 69.2% — the largest single-model jump in Pro history). Previous leaders: Claude Opus 4.8 at 69.2%, Claude Mythos Preview at 77.8%, GPT-5.4 (xHigh) at 59.1%, GPT-5.5 Instant at 58.6%. The gap between Verified and Pro for Fable 5 is ~15 points (95.0% Verified vs 80.3% Pro) — significantly narrower than the ~19 point gap for Opus 4.8, suggesting genuine capability improvement rather than benchmark contamination. Fable 5 at 80.3% Pro means the model autonomously solves 4 out of 5 real GitHub issues end-to-end on harder, less contaminated tasks — a landmark result. Note: the 80.3% figure comes from Anthropic's system card using their own scaffolding; independent evaluation has not yet been published (as of mid-June 2026).
...
**ARC-AGI (Abstraction and Reasoning Corpus)** — visual pattern recognition and abstract reasoning tasks. Each task provides a few input-output grid examples, and the model must infer the transformation rule and apply it to a new input. Designed to test genuine generalization ability, not pattern matching on training data. The original ARC-AGI-1 is largely solved (o3 87.5% with high compute). The harder **ARC-AGI-2** (introduced 2025) is now the frontier benchmark, with Gemini 3.1 Pro reaching 77.1% — the biggest benchmark shift of this period (as of late May 2026). Code-evolution approaches achieved 95.1% (Imbue) as of early 2026, surpassing the human baseline. A key benchmark for measuring progress toward general reasoning.

**Terminal-Bench 2.1** — evaluates agent performance on real-world CLI/terminal tasks (as of late May 2026). GPT-5.5: 82.7%, Gemini 3.5 Flash: 76.2%, Claude Opus 4.8: 74.6%. Measures practical system administration and DevOps capabilities that SWE-bench does not cover.

**MCP Atlas** — evaluates agentic tool use via the Model Context Protocol. Gemini 3.5 Flash leads at 83.6% (as of late May 2026). Measures how effectively models discover, select, and chain MCP tools — critical for production agent deployments.

**ITBench-AA** — enterprise SRE (Site Reliability Engineering) tasks. Frontier systems score below 50% on realistic incident response and infrastructure automation tasks (as of late May 2026). Demonstrates that long-horizon agent reliability in enterprise operations remains low despite strong performance on coding benchmarks.

**Chatbot Arena (LMSYS)** — human preference evaluation at scale. Users submit prompts and compare responses from two anonymous models side-by-side, voting for the better response. Produces Elo ratings from hundreds of thousands of pairwise comparisons. As of late June 2026, Claude Opus 4.8 leads the overall rankings. Fable 5 was available for only 4 days before suspension (June 9-13), preventing meaningful Arena evaluation. The top 10 (including Chinese models like GLM-5 and Kimi K2.5) are separated by only ~44 Elo points. Open-source models have nearly closed the gap with proprietary ones. The most ecologically valid benchmark — measures what users actually prefer in practice. Categories include coding, math, hard prompts, creative writing, and instruction following.

### Benchmark Saturation and the Shift to Harder Evaluations (as of mid-June 2026)

The benchmark landscape is undergoing a structural transition. Classic benchmarks (MMLU, HellaSwag, HumanEval) saturated in 2024-2025. Now even "modern" benchmarks are saturating: GPQA Diamond has reasoning models at 90%+, SWE-bench Verified has Fable 5 at 95%, MMLU-Pro has frontier models at 85-90%. The industry response is a shift toward harder, less gameable evaluations.

The pattern is consistent: a new benchmark is introduced, models improve rapidly on it, scores approach a ceiling within 12-18 months, and the community moves on to a harder successor. MMLU → MMLU-Pro → domain-specific evals. HumanEval → SWE-bench Verified → SWE-bench Pro. ARC-AGI-1 → ARC-AGI-2. Each generation is harder to game through memorization or scaffolding tricks.

For practitioners, this means two things. First, absolute scores on saturating benchmarks are increasingly meaningless — the difference between 93% and 95% on a contaminated benchmark tells you nothing about real-world capability. Second, the benchmarks that matter most are the ones where scores are still low: SWE-bench Pro (80.3% leader), ITBench-AA (<50%), Odysseys (44.5%), RoadmapBench (39.1%). These unsaturated benchmarks reveal genuine capability gaps — especially in long-horizon agent reliability, enterprise operations, and multi-step reasoning under uncertainty.
...
| Coding agents | SWE-bench Pro (preferred), SWE-bench Verified (saturating), HumanEval (saturated) |
...
**Long-horizon agent reliability remains low (as of late May 2026).** While SWE-bench scores exceed 88%, benchmarks measuring multi-step real-world tasks tell a different story. **Odysseys**: 44.5% success at 1.15% trajectory efficiency — agents complete tasks less than half the time and waste most of their steps. **RoadmapBench**: best model achieves 39.1% on version-upgrade tasks requiring multi-step dependency resolution. **ITBench-AA**: frontier systems score below 50% on enterprise SRE tasks. These numbers temper the SWE-bench optimism — controlled coding tasks on curated repositories are fundamentally different from real-world multi-step reliability under uncertainty.
